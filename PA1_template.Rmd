...
```{r Timeseries, echo=TRUE,warning=FALSE}
...
On average, across all the days in the dataset, the 5-minute interval contains the maximum number of steps is __`r IntervalStepsMax`__.
...
The total number of missing values is __`r TotalNumberNA`__.
...
```{r Histogram_NA_rep_Mean, fig.height=5,warning=FALSE}
...
```{r Histogram_NA_rep_Median, fig.height=5,warning=FALSE}
par(mar=c(5,4,1,1),las=1)
...
-----------------------------------------------|----|------|
NAs omitted |__`r StepsMean`__|__`r StepsMedian`__|
NAs replaced with mean by interval|__`r StepsMeanNArepMean`__|__`r StepsMedianNArepMean`__|
NAs replaced with median by interval|__`r StepsMeanNArepMedian`__|__`r StepsMedianNArepMedian`__|
...
```{r TimeseriesComparison, echo=TRUE,warning=FALSE}
library(lattice)
...
__Post Scriptum (to all cheaters' hunters)__: _I have done this assignment for the first time in August 2015, then in april 2016, although I never finished the Reproducible Research Course. This year I dediced to enroll again the Course, so I re-used my work._ 
